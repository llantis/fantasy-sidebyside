...
import { finishSide, finishMatchup, formatRecord, playerKey, round1 } from "./model.js";

const API = 'https://api.sleeper.app/v1';
const PROJ = "https://api.sleeper.com/projections/nfl";
const STATS = "https://api.sleeper.com/stats/nfl";
const STATS_MIN_AGE_MS = 5000;   // a forced refresh right after a poll reuses the last feed
...
// Used by the setup page: does this username exist, and which leagues would we show?
export async function probeSleeper(username) {
  const user = await j(`${API}/user/${encodeURIComponent(username)}`);
  if (!user) return { ok: false, error: `Sleeper user "${username}" not found` };
  const state = await j(`${API}/state/nfl`);
  const leagues = (await j(`${API}/user/${user.user_id}/leagues/nfl/${state.season}`)) ?? [];
  return { ok: true, username: user.username, leagues: leagues.map((l) => ({ id: l.league_id, name: l.name })) };
}

// Live week totals for every NFL player (same undocumented family as projections), keyed by the
// cross-platform playerKey the adapters stamp on starters. Throws on failure; the caller decides.
let statsMem = null;
export async function getLiveStats(season, week) {
  const key = `${season}-${week}`;
  if (statsMem && statsMem.key === key && Date.now() - statsMem.at < STATS_MIN_AGE_MS) return statsMem.data;
  const qs = POSITIONS.map((p) => `position[]=${p}`).join('&');
  const list = await j(`${STATS}/${season}/${week}?season_type=regular&${qs}&order_by=pts_ppr`);
  const data = new Map();
  for (const row of list ?? []) {
    const pl = row?.player;
    if (!pl || !row.stats) continue;
    const k = playerKey({ name: `${pl.first_name ?? ''} ${pl.last_name ?? ''}`.trim(), pos: pl.position, team: row.team });
    data.set(k, { pos: pl.position, team: row.team, updatedAt: row.updated_at ?? null, stats: row.stats });
  }
  statsMem = { key, at: Date.now(), data };
  return data;
}
